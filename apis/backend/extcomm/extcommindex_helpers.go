...
func (r *EXTCOMMIndex) GetCondition(t condition.ConditionType) condition.Condition {
...
func (r *EXTCOMMIndex) ValidateSyntax(s string) field.ErrorList {
	...
	if GetEXTCOMMType(r.Spec.Type) == ExtendedCommunityType_Invalid {
	...
	if r.Spec.MinID != nil {
		if err := validateEXTCOMMID(GetEXTCOMMType(r.Spec.Type), *r.Spec.MinID); err != nil {
			allErrs = append(allErrs, field.Invalid(
	...
	if r.Spec.MaxID != nil {
		if err := validateEXTCOMMID(GetEXTCOMMType(r.Spec.Type), *r.Spec.MaxID); err != nil {
			allErrs = append(allErrs, field.Invalid(

...
func (r *EXTCOMMEntry) GetCondition(t conditionv1alpha1.ConditionType) conditionv1alpha1.Condition {
...
func (r *EXTCOMMEntry) SetSpec(s any) {
	if spec, ok := s.(EXTCOMMEntrySpec); ok {
		r.Spec = spec
	}
}
...
func BuildEXTCOMMEntry(meta metav1.ObjectMeta, spec *EXTCOMMEntrySpec, status *EXTCOMMEntryStatus) *EXTCOMMEntry {
	aspec := EXTCOMMEntrySpec{}
	if spec != nil {
		aspec = *spec
	}
	astatus := EXTCOMMEntryStatus{}
	if status != nil {
		astatus = *status
	}
	return &EXTCOMMEntry{

...
type IPClaimSpec struct {
	...
	// +optional
	CreatePrefix *bool `json:"createPrefix,omitempty" protobuf:"varint,7,opt,name=createPrefix"`
	...
	// +optional
	PrefixLength *uint32 `json:"prefixLength,omitempty" protobuf:"varint,8,opt,name=prefixLength"`
	// AddressFamily defines the address family for the IP claim
	// +kubebuilder:validation:Enum=`ipv4`;`ipv6`
	// +kubebuilder:validation:Optional
	// +optional
	AddressFamily *iputil.AddressFamily `json:"addressFamily,omitempty" protobuf:"bytes,9,opt,name=addressFamily"`
	...
	// +kubebuilder:validation:Optional
	// +optional
	Idx *uint32 `json:"idx,omitempty" protobuf:"varint,10,opt,name=idx"`
	// ClaimLabels define the user defined labels and selector labels used
	// in resource claim
	commonv1alpha1.ClaimLabels `json:",inline" protobuf:"bytes,11,opt,name=claimLabels"`
}
...
type IPClaimStatus struct {
	...
	// +optional
	Range *string `json:"range,omitempty" protobuf:"bytes,2,opt,name=range"`
	...
	// +optional
	DefaultGateway *string `json:"defaultGateway,omitempty" protobuf:"bytes,5,opt,name=defaultGateway"`
	// ExpiryTime defines when the claim expires
	// +kubebuilder:validation:Optional
	// +optional
	ExpiryTime *string `json:"expiryTime,omitempty" protobuf:"bytes,6,opt,name=expiryTime"`
}
...
type IPClaim struct {
	...
	Status IPClaimStatus `json:"status,omitempty" protobuf:"bytes,3,opt,name=status"`
}

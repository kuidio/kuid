...
var _ resource.InternalObject = &VLANIndex{}
var _ resource.ObjectList = &VLANIndexList{}
var _ resource.ObjectWithStatusSubResource = &VLANIndex{}
var _ resource.StatusSubResource = &VLANIndexStatus{}
...
func (r *VLANIndex) IsEqual(ctx context.Context, obj, old runtime.Object) bool {
	newobj := obj.(*VLANIndex)
	oldobj := old.(*VLANIndex)
...
func (VLANIndexStatus) SubResourceName() string {
	return fmt.Sprintf("%s/%s", VLANIndexPlural, "status")
}
...
func (r *VLANIndex) FieldSelector() func(ctx context.Context, fieldSelector fields.Selector) (resource.Filter, error) {
	return func(ctx context.Context, fieldSelector fields.Selector) (resource.Filter, error) {
		var filter *VLANIndexFilter

		// add the namespace to the list
		namespace, ok := genericapirequest.NamespaceFrom(ctx)
		if fieldSelector == nil {
			if ok {
				return &VLANIndexFilter{Namespace: namespace}, nil
			}
			return filter, nil
		}
		requirements := fieldSelector.Requirements()
		for _, requirement := range requirements {
			filter = &VLANIndexFilter{}
		...
		if ok {
			if filter != nil {
				...
				filter = &VLANIndexFilter{Namespace: namespace}
			}
			return filter, nil
		}

		return &VLANIndexFilter{}, nil
	}

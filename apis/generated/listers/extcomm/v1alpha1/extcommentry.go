...
func NewEXTCOMMEntryLister(indexer cache.Indexer) EXTCOMMEntryLister {
	return &eXTCOMMEntryLister{indexer: indexer}
}

// List lists all EXTCOMMEntries in the indexer.
func (s *eXTCOMMEntryLister) List(selector labels.Selector) (ret []*v1alpha1.EXTCOMMEntry, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.EXTCOMMEntry))
	})
	return ret, err
}
...
func (s *eXTCOMMEntryLister) EXTCOMMEntries(namespace string) EXTCOMMEntryNamespaceLister {
	return eXTCOMMEntryNamespaceLister{indexer: s.indexer, namespace: namespace}
}
...
// List lists all EXTCOMMEntries in the indexer for a given namespace.
func (s eXTCOMMEntryNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.EXTCOMMEntry, err error) {
...
// Get retrieves the EXTCOMMEntry from the indexer for a given namespace and name.
func (s eXTCOMMEntryNamespaceLister) Get(name string) (*v1alpha1.EXTCOMMEntry, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("extcommentry"), name)
	}
	return obj.(*v1alpha1.EXTCOMMEntry), nil
}

...
func NewEXTCOMMClaimLister(indexer cache.Indexer) EXTCOMMClaimLister {
	return &eXTCOMMClaimLister{indexer: indexer}
}

// List lists all EXTCOMMClaims in the indexer.
func (s *eXTCOMMClaimLister) List(selector labels.Selector) (ret []*v1alpha1.EXTCOMMClaim, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.EXTCOMMClaim))
	})
	return ret, err
}
...
func (s *eXTCOMMClaimLister) EXTCOMMClaims(namespace string) EXTCOMMClaimNamespaceLister {
	return eXTCOMMClaimNamespaceLister{indexer: s.indexer, namespace: namespace}
}
...
// List lists all EXTCOMMClaims in the indexer for a given namespace.
func (s eXTCOMMClaimNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.EXTCOMMClaim, err error) {
...
// Get retrieves the EXTCOMMClaim from the indexer for a given namespace and name.
func (s eXTCOMMClaimNamespaceLister) Get(name string) (*v1alpha1.EXTCOMMClaim, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("extcommclaim"), name)
	}
	return obj.(*v1alpha1.EXTCOMMClaim), nil
}

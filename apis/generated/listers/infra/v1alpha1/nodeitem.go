...
func NewNodeItemLister(indexer cache.Indexer) NodeItemLister {
	return &nodeItemLister{indexer: indexer}
}

// List lists all NodeItems in the indexer.
func (s *nodeItemLister) List(selector labels.Selector) (ret []*v1alpha1.NodeItem, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.NodeItem))
	})
	return ret, err
}
...
func (s *nodeItemLister) NodeItems(namespace string) NodeItemNamespaceLister {
	return nodeItemNamespaceLister{indexer: s.indexer, namespace: namespace}
}
...
// List lists all NodeItems in the indexer for a given namespace.
func (s nodeItemNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.NodeItem, err error) {
...
// Get retrieves the NodeItem from the indexer for a given namespace and name.
func (s nodeItemNamespaceLister) Get(name string) (*v1alpha1.NodeItem, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("nodeitem"), name)
	}
	return obj.(*v1alpha1.NodeItem), nil
}

...
RUN --mount=type=cache,target=/root/.cache/go-build \
    --mount=type=ssh \
    CGO_ENABLED=0 GOOS=${TARGETOS:-linux} GOARCH=${TARGETARCH} go build -a -o kuid-server main.go
...
#
COPY --from=builder --chown=$USERID:$USERID /workspace/kuid-server /app/
WORKDIR /app
# from now on, run as the unprivileged user
USER $USERID
ENTRYPOINT ["/app/kuid-server"]

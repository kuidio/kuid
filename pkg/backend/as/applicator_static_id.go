...
type staticASApplicator struct {
	name string
	applicator
	claimID        *uint32
...
func (r *staticASApplicator) Validate(ctx context.Context, claim *asbev1alpha1.ASClaim) error {
...
func (r *staticASApplicator) reclaimID(ctx context.Context, claim *asbev1alpha1.ASClaim) error {
	// reclaim ID
...
func (r *staticASApplicator) getParentContext(ctx context.Context, claim *asbev1alpha1.ASClaim) error {
	...
	if err == nil {
		...
		claimType := asbev1alpha1.GetClaimTypeFromString(labels[backend.KuidASClaimTypeKey])
		if claimType == asbev1alpha1.ASClaimType_Range {
	...
	for _, parentEntry := range parentEntries {
		...
		parentClaimType := asbev1alpha1.GetClaimTypeFromString(labels[backend.KuidASClaimTypeKey])
		if parentClaimType == asbev1alpha1.ASClaimType_Range {
			r.parentTreeName = labels[backend.KuidClaimNameKey]
			break
		} else {
			log.Error("got parent which is not a range", "entry", parentEntry)
			return fmt.Errorf("got parent which is not a range %s", labels[backend.KuidASClaimTypeKey])
		}
	}
	return nil
}

func (r *staticASApplicator) Apply(ctx context.Context, claim *asbev1alpha1.ASClaim) error {
...
func (r *staticASApplicator) Delete(ctx context.Context, claim *asbev1alpha1.ASClaim) error {

...
func (r *bestore) Restore(ctx context.Context, k store.Key) error {
	...
	if err := r.restoreClaims(ctx, cacheCtx, curEntries, asbev1alpha1.ASClaimType_Range, claimmap); err != nil {
		return err
	}
	if err := r.restoreClaims(ctx, cacheCtx, curEntries, asbev1alpha1.ASClaimType_StaticID, claimmap); err != nil {
		return err
	}
	if err := r.restoreClaims(ctx, cacheCtx, curEntries, asbev1alpha1.ASClaimType_DynamicID, claimmap); err != nil {
		return err
	}
...
func (r *bestore) SaveAll(ctx context.Context, k store.Key) error {
	...
	for _, newEntry := range newEntries {
		newEntry := newEntry
		found := false
		var entry *asbev1alpha1.ASEntry
...
func (r *bestore) getEntriesFromCache(ctx context.Context, k store.Key) ([]*asbev1alpha1.ASEntry, error) {
	log := log.FromContext(ctx).With("key", k.String())
	...
	entries := make([]*asbev1alpha1.ASEntry, 0, cacheCtx.Size())
	...
	for _, entry := range cacheCtx.tree.GetAll() {
		...
		entry := entry
		entries = append(entries, asbev1alpha1.GetASEntry(ctx, k, "", entry.ID().String(), entry.Labels()))
	}
	...
	cacheCtx.ranges.List(ctx, func(ctx context.Context, key store.Key, t *table32.Table32) {
		for _, entry := range t.GetAll() {
			...
			entry := entry
			entries = append(entries, asbev1alpha1.GetASEntry(ctx, k, key.Name, entry.ID().String(), entry.Labels()))
		}
	})
...
func (r *bestore) listEntries(ctx context.Context, k store.Key) ([]*asbev1alpha1.ASEntry, error) {
	...
	entryList := asbev1alpha1.ASEntryList{}
	if err := r.client.List(ctx, &entryList, opt...); err != nil {
		return nil, err
	}
	entries := []*asbev1alpha1.ASEntry{}
...
func (r *bestore) listClaims(ctx context.Context, k store.Key) (map[string]*asbev1alpha1.ASClaim, error) {
	opt := []client.ListOption{
		/*
			client.MatchingFields{
				"spec.networkInstance": k.Name,
	...
	claims := asbev1alpha1.ASClaimList{}
	if err := r.client.List(ctx, &claims, opt...); err != nil {
		return nil, err
	}

	claimmap := map[string]*asbev1alpha1.ASClaim{}
...
func (r *bestore) restoreMinMaxRanges(ctx context.Context, cacheCtx *CacheContext, entries []*asbev1alpha1.ASEntry, index *asbev1alpha1.ASIndex) error {
	for i := len(entries) - 1; i >= 0; i-- {
		entry := entries[i]
		if entry.Spec.Owner.Group == asbev1alpha1.SchemeGroupVersion.Group &&
			entry.Spec.Owner.Version == asbev1alpha1.SchemeGroupVersion.Version &&
			entry.Spec.Owner.Kind == asbev1alpha1.ASIndexKind {

			entries = append(entries[:i], entries[i+1:]...)
		}
	}

	if index.Spec.MinID != nil && *index.Spec.MinID != asbev1alpha1.ASID_Min {
		claim := index.GetMinClaim()
		if err := r.restoreClaim(ctx, cacheCtx, claim); err != nil {
			return err
		}
	}
	if index.Spec.MaxID != nil && *index.Spec.MaxID != asbev1alpha1.ASID_Max {
...
func (r *bestore) restoreClaims(ctx context.Context, cacheCtx *CacheContext, entries []*asbev1alpha1.ASEntry, claimType asbev1alpha1.ASClaimType, claimmap map[string]*asbev1alpha1.ASClaim) error {

	for i := len(entries) - 1; i >= 0; i-- {
		entry := entries[i]
		if entry.Spec.Owner.Group == asbev1alpha1.SchemeGroupVersion.Group &&
			entry.Spec.Owner.Version == asbev1alpha1.SchemeGroupVersion.Version &&
			entry.Spec.Owner.Kind == asbev1alpha1.ASClaimKind {
...
func (r *bestore) restoreClaim(ctx context.Context, cacheCtx *CacheContext, claim *asbev1alpha1.ASClaim) error {

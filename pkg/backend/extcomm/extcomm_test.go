...
func Test(t *testing.T) {
	...
	for name, tc := range tests {
		...
		testTypes := []string{
			string(extcommbev1alpha1.ExtendedCommunityType_2byteAS),
			string(extcommbev1alpha1.ExtendedCommunityType_4byteAS),
			string(extcommbev1alpha1.ExtendedCommunityType_IPv4Address),
			//string(extcommbev1alpha1.ExtendedCommunityType_Opaque),
		}
		...
		for _, testType := range testTypes {
			t.Run(name, func(t *testing.T) {
				...
				for _, v := range tc.ctxs {
					v := v
					var claim *extcommbev1alpha1.EXTCOMMClaim
					var err error

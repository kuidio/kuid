...
func (r *staticApplicator) reclaimID(ctx context.Context, claim backend.ClaimObject) error {
	// reclaim ID
...
func (r *staticApplicator) getParentContext(ctx context.Context, claim backend.ClaimObject) error {
	log := log.FromContext(ctx).With("name", claim.GetName())

	entry, err := r.cacheCtx.tree.Get(claim.GetStaticTreeID(r.cacheCtx.Type()))
	if err == nil {
		// entry exists
		labels := entry.Labels()
	...
	parentEntries := r.cacheCtx.tree.Parents(claim.GetStaticTreeID(r.cacheCtx.Type()))
...
func (r *staticApplicator) Apply(ctx context.Context, claim backend.ClaimObject) error {
	...
	if r.parentTreeName == "" {
		...
		if r.claimID != nil {
			...
			if err := r.cacheCtx.tree.ClaimID(claim.GetStaticTreeID(r.cacheCtx.Type()), claim.GetClaimLabels()); err != nil {
				return err
			}
		}
	} else {

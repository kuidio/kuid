...
func Test(t *testing.T) {
	...
	for name, tc := range tests {
		...
		testTypes := []string{
			string(genidbev1alpha1.GENIDType_16bit),
			string(genidbev1alpha1.GENIDType_32bit),
			//string(genidbev1alpha1.GENIDType_48bit),
			//string(genidbev1alpha1.GENIDType_64bit),
		}
		...
		for _, testType := range testTypes {
			t.Run(name, func(t *testing.T) {
				...
				for _, v := range tc.ctxs {
					v := v
					var claim *genidbev1alpha1.GENIDClaim
					var err error

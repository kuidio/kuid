...
func (r *be) CreateIndex(ctx context.Context, obj runtime.Object) error {
	cr, ok := obj.(*vxlanbev1alpha1.VXLANIndex)
	if !ok {
		return fmt.Errorf("cannot create index expecting %s, got %s", vxlanbev1alpha1.VXLANIndexKind, reflect.TypeOf(obj).Name())
	}
...
func (r *be) DeleteIndex(ctx context.Context, obj runtime.Object) error {
	cr, ok := obj.(*vxlanbev1alpha1.VXLANIndex)
	if !ok {
		return fmt.Errorf("cannot delete index expecting %s, got %s", vxlanbev1alpha1.VXLANIndexKind, reflect.TypeOf(obj).Name())
	}
...
func (r *be) Claim(ctx context.Context, obj runtime.Object) error {
	claim, ok := obj.(*vxlanbev1alpha1.VXLANClaim)
	if !ok {
		return fmt.Errorf("cannot claim expecting %s, got %s", vxlanbev1alpha1.VXLANClaimKind, reflect.TypeOf(obj).Name())
	}
...
func (r *be) Release(ctx context.Context, obj runtime.Object) error {
	claim, ok := obj.(*vxlanbev1alpha1.VXLANClaim)
	if !ok {
		return fmt.Errorf("cannot delete claim expecting %s, got %s", vxlanbev1alpha1.VXLANIndexKind, reflect.TypeOf(obj).Name())
	}
...
func getApplicator(_ context.Context, cacheCtx *CacheContext, claim *vxlanbev1alpha1.VXLANClaim) (Applicator, error) {
	claimType := claim.GetClaimType()
	var a Applicator
	switch claimType {
	case vxlanbev1alpha1.VXLANClaimType_DynamicID:
		a = &dynamicVXLANApplicator{name: string(claimType), applicator: applicator{cacheCtx: cacheCtx}}
	case vxlanbev1alpha1.VXLANClaimType_StaticID:
		a = &staticVXLANApplicator{name: string(claimType), applicator: applicator{cacheCtx: cacheCtx}}
	case vxlanbev1alpha1.VXLANClaimType_Range:
		a = &rangeVXLANApplicator{name: string(claimType), applicator: applicator{cacheCtx: cacheCtx}}
	default:

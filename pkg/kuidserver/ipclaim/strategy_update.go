...
func (r *strategy) Update(ctx context.Context, key types.NamespacedName, obj, old runtime.Object, dryrun bool) (runtime.Object, error) {
	...
	newIPClaim, ok := obj.(*ipambe1v1alpha1.IPClaim)
	if !ok {
		return obj, fmt.Errorf("unexpected new object, expecting: %s, got: %s", ipambe1v1alpha1.IPClaimKind, reflect.TypeOf(obj))
	}
	oldIPClaim, ok := old.(*ipambe1v1alpha1.IPClaim)
	if !ok {
		return obj, fmt.Errorf("unexpected old object, expecting: %s, got: %s", ipambe1v1alpha1.IPClaimKind, reflect.TypeOf(obj))
	}

	newHash, err := newIPClaim.CalculateHash()
	if err != nil {
		return obj, err
	}
	oldHash, err := oldIPClaim.CalculateHash()
	if err != nil {
		return obj, err
	}

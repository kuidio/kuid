...
func (r *strategy) Update(ctx context.Context, key types.NamespacedName, obj, old runtime.Object, dryrun bool) (runtime.Object, error) {
	...
	newIPEntry, ok := obj.(*ipambe1v1alpha1.IPEntry)
	if !ok {
		return obj, fmt.Errorf("unexpected new object, expecting: %s, got: %s", ipambe1v1alpha1.IPEntryKind, reflect.TypeOf(obj))
	}
	oldIPEntry, ok := old.(*ipambe1v1alpha1.IPEntry)
	if !ok {
		return obj, fmt.Errorf("unexpected old object, expecting: %s, got: %s", ipambe1v1alpha1.IPEntryKind, reflect.TypeOf(obj))
	}

	newHash, err := newIPEntry.CalculateHash()
	if err != nil {
		return obj, err
	}
	oldHash, err := oldIPEntry.CalculateHash()
	if err != nil {
		return obj, err
	}

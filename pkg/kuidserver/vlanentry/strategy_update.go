...
func (r *strategy) Update(ctx context.Context, key types.NamespacedName, obj, old runtime.Object, dryrun bool) (runtime.Object, error) {
	...
	newObj, ok := obj.(*vlanbe1v1alpha1.VLANEntry)
	if !ok {
		return obj, fmt.Errorf("unexpected new object, expecting: %s, got: %s", vlanbe1v1alpha1.VLANEntryKind, reflect.TypeOf(obj))
	}
	oldObj, ok := old.(*vlanbe1v1alpha1.VLANEntry)
	if !ok {
		return obj, fmt.Errorf("unexpected old object, expecting: %s, got: %s", vlanbe1v1alpha1.VLANEntryKind, reflect.TypeOf(obj))
	}

...
func init() {
	reconcilers.Register("ipamipindex", &reconciler{})
}

const (
	crName         = "ipIndex"
	controllerName = "IPAMIPIndexController"
...
//+kubebuilder:rbac:groups=ipindex.ipam.be.kuid.dev,resources=ipindex,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=ipindex.ipam.be.kuid.dev,resources=ipindex/status,verbs=get;update;patch
...
func (r *reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if !cr.GetDeletionTimestamp().IsZero() {
		if err := r.be.DeleteIndex(ctx, cr); err != nil {
			r.handleError(ctx, cr, "cannot delete ipindex", err)
	...
	if err := r.be.CreateIndex(ctx, cr); err != nil {
		r.handleError(ctx, cr, "cannot create ipindex", err)

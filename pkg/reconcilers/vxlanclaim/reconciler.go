...
func init() {
	reconcilers.Register("vxlanclaim", &reconciler{})
}

const (
	crName         = "vxlanClaim"
	controllerName = "VXLANClaimController"
	finalizer      = "vxlanclaim.vxlan.be.kuid.dev/finalizer"
	// errors
...
func (r *reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.be.Claim(ctx, cr); err != nil {
		r.handleError(ctx, cr, "cannot claim vxlan", err)
...
func (r *reconciler) handleError(ctx context.Context, cr *vxlanbev1alpha1.VXLANClaim, msg string, err error) {
	log := log.FromContext(ctx)
	if err == nil {

...
func init() {
	reconcilers.Register("vlanindex", &reconciler{})
}

const (
	crName         = "vlanindex"
	controllerName = "VLANIndexController"
	finalizer      = "vlanindex.vlan.res.kuid.dev/finalizer"
	// errors
	errGetCr        = "cannot get cr"
	errUpdateStatus = "cannot update status"
)

//+kubebuilder:rbac:groups=vlanindex.vlan.res.kuid.dev,resources=vlanindices,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=vlanindex.vlan.res.kuid.dev,resources=vlanindices/status,verbs=get;update;patch
...
func (r *reconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, c interface{}) (map[schema.GroupVersionKind]chan event.GenericEvent, error) {
	...
	return nil, ctrl.NewControllerManagedBy(mgr).
		...
		Watches(&vlanbev1alpha1.VLANIndex{},
			&eventhandler.IPEntryEventHandler{
				Client:  mgr.GetClient(),
				ObjList: &vlanbev1alpha1.VLANIndexList{},
			}).
		Complete(r)
}
...
type reconciler struct {
	...
	be        backend.Backend[*vlan.CacheContext]
}
...
func (r *reconciler) handleError(ctx context.Context, cr *vlanbev1alpha1.VLANIndex, msg string, err error) {
	log := log.FromContext(ctx)
	if err == nil {
...
func (r *reconciler) deleteIndex(ctx context.Context, cr *vlanbev1alpha1.VLANIndex) error {
...
func (r *reconciler) applyIndex(ctx context.Context, cr *vlanbev1alpha1.VLANIndex) error {

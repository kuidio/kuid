...
func init() {
	reconcilers.Register(extcomm.GroupName, extcommbev1alpha1.EXTCOMMIndexKind, &reconciler{})
}

const (
	reconcilerName = "EXTCOMMIndexController"
	finalizer      = "extcommindex.extcomm.be.kuid.dev/finalizer"
	// errors
...
func (r *reconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, c interface{}) (map[schema.GroupVersionKind]chan event.GenericEvent, error) {
	...
	r.be = cfg.Backends[extcommbev1alpha1.SchemeGroupVersion.Group]
...
func (r *reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	index := &extcommbev1alpha1.EXTCOMMIndex{}
	...
	if !index.GetDeletionTimestamp().IsZero() {
		...
		// takes care of this
		intIndex := &extcomm.EXTCOMMIndex{}
		if err := extcommbev1alpha1.Convert_v1alpha1_EXTCOMMIndex_To_extcomm_EXTCOMMIndex(index, intIndex, nil); err != nil {
		...
		if err := extcommbev1alpha1.Convert_extcomm_EXTCOMMIndex_To_v1alpha1_EXTCOMMIndex(intIndex, index, nil); err != nil {
			return ctrl.Result{Requeue: true},
...
func (r *reconciler) handleSuccess(ctx context.Context, index *extcommbev1alpha1.EXTCOMMIndex) error {
	// take a snapshot of the current object
	patch := client.MergeFrom(index.DeepCopy())
	// update status
	index.SetConditions(condv1alpha1.Ready())
	r.recorder.Eventf(index, corev1.EventTypeNormal, extcommbev1alpha1.EXTCOMMIndexKind, "ready")
...
func (r *reconciler) handleError(ctx context.Context, index *extcommbev1alpha1.EXTCOMMIndex, msg string, err error) error {
	log := log.FromContext(ctx)
	// take a snapshot of the current object
	patch := client.MergeFrom(index.DeepCopy())
	...
	log.Error(msg)
	r.recorder.Eventf(index, corev1.EventTypeWarning, extcommbev1alpha1.EXTCOMMIndexKind, msg)
